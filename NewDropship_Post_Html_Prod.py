...
# Read Excel files
python_script_cleaned = sys.argv[1]
initial_raw_file = sys.argv[2]
...
df3_result['Tags']= df3_result['Tags'].apply(lambda x: x.replace('not_update_CA', f'DZ2AU_{current_date}') if 'not_update_CA' in x else x)
...
df3_result['Vendor'] = df3_result.apply(lambda row: 'DZ2AUV' if row['Variant SKU'].startswith('V') else 'DZ2AU', axis=1)
