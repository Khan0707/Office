# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 08:37:01 2023
...
python_script_cleaned_base = r"D:\BackendData\Soga\30_11_Soga_CleanedDesc_xyz_HTML.xlsx"
initial_raw_file_base= r"D:\BackendData\Soga\30_11_Soga_RawExport_xyz.xlsx"
### NZ: 
...
df['Body HTML'] = df['Body HTML'].str.replace('<li>', '')
...
duplicate_indices = df3[df3.duplicated('Variant SKU')]['Variant SKU'].index
...
vendor_mapping = {'idropship': 'GODIAU', 'vidaxl':'GOAUAD', 'wefullfill':'Vibe Geeks', 'bigbuy':'PDBB', 'matterhorn':'GOEFASH','bangood':"Goslash-DIOW", 'Soga':'Soga'}
...
def add_package_includes(row):
    ...
    if '<br><br><strong>Package Includes:</strong>' not in row['Body HTML']:
...
df3_result = df3_result.drop(2581) #index=list(range(212, 226)) + list(range(2035, 2041))+[4050])

df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('This Product Only Includes The Plant Frame, No Other Items Included.','')

df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('Home<br> • Commercial','Home Commercial')
df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('Home<br> • Office','Home Office')
df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('Home<br> • Events','Home Events')
df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('Home<br> • Outdoor','Home Outdoor')
df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('<br> Usage: <br> •','<br> Usage:')
#df3_result['Body HTML'].str.contains('<br> Usage: <br> •').sum()
...
#df3_result['Body HTML'].str.contains('<strong>Features:</strong>').sum()
...
def replace_before_colon(text):
    ...
    return text
df3_result['Body HTML'] = df3_result['Body HTML'].apply(replace_before_colon)
...
# Replace '•M' with '• M' where M can be any number or letter
#df3_result['Body HTML'] = df3_result['Body HTML'].apply(lambda text: re.sub(r'•([A-Za-z0-9])', r'• \1', text))


# Assuming 'ID' is the column you want to check for duplicates
duplicate_cases = df3_result[df3_result.duplicated(subset='ID', keep=False)]

df3_result = df3_result.drop_duplicates(subset='ID', keep='first')


# stop_words = ['CM', 'Cm X',' X ', 'KG']


# df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('CM','cm')
# df3_result['Body HTML'] = df3_result['Body HTML'].str.replace(' X ',' x ')
# df3_result['Body HTML'] = df3_result['Body HTML'].str.replace('KG','kg')

# df3_result['Title'] = df3_result['Title'].str.replace('CM','cm')
# df3_result['Title'] = df3_result['Title'].str.replace('X ',' x ')
# df3_result['Title'] = df3_result['Title'].str.replace('KG','kg')
# df3_result['Title'] = df3_result['Title'].str.replace('Mm','mm')
# df3_result['Title'] = df3_result['Title'].str.replace(' Of ',' of ')
...
df3_result['Title'] = ensure_stop_words_lower_specific(df3_result['Title'], stop_words)
df3_result['Body HTML'] = ensure_stop_words_lower_specific(df3_result['Body HTML'], stop_words)
